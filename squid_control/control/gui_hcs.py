...
class OctopiGUI(QMainWindow):
    ...
    def __init__(self, is_simulation = False, *args, **kwargs):
        ...
        if is_simulation:
            ...
            try:
                if CONFIG.SUPPORT_LASER_AUTOFOCUS:
                    ...
                    sn_camera_focus = camera_fc.get_sn_by_model(CONFIG.FOCUS_CAMERA_MODEL)
        ...
        self.navigationViewer = core.NavigationViewer(sample=str(CONFIG.WELLPLATE_FORMAT)+' well plate')
        '''
        ...
        if CONFIG.HAS_ENCODER_X == True:
            self.navigationController.configure_encoder(0, (CONFIG.SCREW_PITCH_X_MM * 1000) / CONFIG.ENCODER_RESOLUTION_UM_X, CONFIG.ENCODER_FLIP_DIR_X)
            self.navigationController.set_pid_control_enable(0, CONFIG.ENABLE_PID_X)
        if CONFIG.HAS_ENCODER_Y == True:
            self.navigationController.configure_encoder(1, (CONFIG.SCREW_PITCH_Y_MM * 1000) / CONFIG.ENCODER_RESOLUTION_UM_Y, CONFIG.ENCODER_FLIP_DIR_Y)
            self.navigationController.set_pid_control_enable(1, CONFIG.ENABLE_PID_Y)
        if CONFIG.HAS_ENCODER_Z == True:
            self.navigationController.configure_encoder(2, (CONFIG.SCREW_PITCH_Z_MM * 1000) / CONFIG.ENCODER_RESOLUTION_UM_Z, CONFIG.ENCODER_FLIP_DIR_Z)
        ...
        self.microcontroller.set_dac80508_scaling_factor_for_illumination(CONFIG.ILLUMINATION_INTENSITY_FACTOR)
        ...
        if CONFIG.SUPPORT_LASER_AUTOFOCUS:
            ...
            self.laserAutofocusController = core.LaserAutofocusController(self.microcontroller,self.camera_focus,self.liveController_focus_camera,self.navigationController,has_two_interfaces=CONFIG.HAS_TWO_INTERFACES,use_glass_top=CONFIG.USE_GLASS_TOP,look_for_cache=False)
