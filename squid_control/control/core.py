...
from qtpy.QtWidgets import QMainWindow, QWidget, QGridLayout, QDesktopWidget, QFrame, QVBoxLayout
...
class ObjectiveStore:
    def __init__(self, objectives_dict = CONFIG.OBJECTIVES, default_objective = CONFIG.DEFAULT_OBJECTIVE):
...
class StreamHandler(QObject):
    ...
    def __init__(self,crop_width=CONFIG.Acquisition.CROP_WIDTH,crop_height=CONFIG.Acquisition.CROP_HEIGHT,display_resolution_scaling=1):
...
class LiveController(QObject):
    ...
    def set_illumination(self,illumination_source,intensity):
        if illumination_source < 10: # LED matrix
            self.microcontroller.set_illumination_led_matrix(illumination_source,r=(intensity/100)*CONFIG.LED_MATRIX_R_FACTOR,g=(intensity/100)*CONFIG.LED_MATRIX_G_FACTOR,b=(intensity/100)*CONFIG.LED_MATRIX_B_FACTOR)
        else:
    ...
    def start_live(self):
        ...
        if self.trigger_mode == TriggerModeSetting.SOFTWARE or ( self.trigger_mode == TriggerModeSetting.HARDWARE and self.use_internal_timer_for_hardware_trigger ):
    ...
    def set_trigger_mode(self,mode):
        if mode == TriggerModeSetting.SOFTWARE.value:
            if self.is_live and ( self.trigger_mode == TriggerModeSetting.HARDWARE and self.use_internal_timer_for_hardware_trigger ):
                self._stop_triggerred_acquisition()
...
class NavigationController(QObject):
    ...
    def move_x(self,delta):
        self.microcontroller.move_x_usteps(int(delta/(CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X))))

    def move_y(self,delta):
        self.microcontroller.move_y_usteps(int(delta/(CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y))))

    def move_z(self,delta):
        self.microcontroller.move_z_usteps(int(delta/(CONFIG.SCREW_PITCH_Z_MM/(self.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z))))

    def move_x_to(self,delta):
        self.microcontroller.move_x_to_usteps(CONFIG.STAGE_MOVEMENT_SIGN_X*int(delta/(CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X))))

    def move_y_to(self,delta):
        self.microcontroller.move_y_to_usteps(CONFIG.STAGE_MOVEMENT_SIGN_Y*int(delta/(CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y))))

    def move_z_to(self,delta):
        self.microcontroller.move_z_to_usteps(CONFIG.STAGE_MOVEMENT_SIGN_Z*int(delta/(CONFIG.SCREW_PITCH_Z_MM/(self.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z))))
    ...
    def update_pos(self,microcontroller):
        ...
        if CONFIG.USE_ENCODER_X:
            self.x_pos_mm = x_pos*CONFIG.ENCODER_POS_SIGN_X*CONFIG.ENCODER_STEP_SIZE_X_MM
        else:
            self.x_pos_mm = x_pos*CONFIG.STAGE_POS_SIGN_X*(CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X))
        if CONFIG.USE_ENCODER_Y:
            self.y_pos_mm = y_pos*CONFIG.ENCODER_POS_SIGN_Y*CONFIG.ENCODER_STEP_SIZE_Y_MM
        else:
            self.y_pos_mm = y_pos*CONFIG.STAGE_POS_SIGN_Y*(CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y))
        if CONFIG.USE_ENCODER_Z:
            self.z_pos_mm = z_pos*CONFIG.ENCODER_POS_SIGN_Z*CONFIG.ENCODER_STEP_SIZE_Z_MM
        else:
            self.z_pos_mm = z_pos*CONFIG.STAGE_POS_SIGN_Z*(CONFIG.SCREW_PITCH_Z_MM/(self.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z))
        if CONFIG.USE_ENCODER_THETA:
            self.theta_pos_rad = theta_pos*CONFIG.ENCODER_POS_SIGN_THETA*CONFIG.ENCODER_STEP_SIZE_THETA
        else:
            self.theta_pos_rad = theta_pos*CONFIG.STAGE_POS_SIGN_THETA*(2*math.pi/(self.theta_microstepping*CONFIG.FULLSTEPS_PER_REV_THETA))
    ...
    def set_x_limit_pos_mm(self,value_mm):
        if CONFIG.STAGE_MOVEMENT_SIGN_X > 0:
            self.microcontroller.set_lim(LIMIT_CODE.X_POSITIVE,int(value_mm/(CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X))))
        else:
            self.microcontroller.set_lim(LIMIT_CODE.X_NEGATIVE,CONFIG.STAGE_MOVEMENT_SIGN_X*int(value_mm/(CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X))))

    def set_x_limit_neg_mm(self,value_mm):
        if CONFIG.STAGE_MOVEMENT_SIGN_X > 0:
            self.microcontroller.set_lim(LIMIT_CODE.X_NEGATIVE,int(value_mm/(CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X))))
        else:
            self.microcontroller.set_lim(LIMIT_CODE.X_POSITIVE,CONFIG.STAGE_MOVEMENT_SIGN_X*int(value_mm/(CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X))))

    def set_y_limit_pos_mm(self,value_mm):
        if CONFIG.STAGE_MOVEMENT_SIGN_Y > 0:
            self.microcontroller.set_lim(LIMIT_CODE.Y_POSITIVE,int(value_mm/(CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y))))
        else:
            self.microcontroller.set_lim(LIMIT_CODE.Y_NEGATIVE,CONFIG.STAGE_MOVEMENT_SIGN_Y*int(value_mm/(CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y))))

    def set_y_limit_neg_mm(self,value_mm):
        if CONFIG.STAGE_MOVEMENT_SIGN_Y > 0:
            self.microcontroller.set_lim(LIMIT_CODE.Y_NEGATIVE,int(value_mm/(CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y))))
        else:
            self.microcontroller.set_lim(LIMIT_CODE.Y_POSITIVE,CONFIG.STAGE_MOVEMENT_SIGN_Y*int(value_mm/(CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y))))

    def set_z_limit_pos_mm(self,value_mm):
        if CONFIG.STAGE_MOVEMENT_SIGN_Z > 0:
            self.microcontroller.set_lim(LIMIT_CODE.Z_POSITIVE,int(value_mm/(CONFIG.SCREW_PITCH_Z_MM/(self.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z))))
        else:
            self.microcontroller.set_lim(LIMIT_CODE.Z_NEGATIVE,CONFIG.STAGE_MOVEMENT_SIGN_Z*int(value_mm/(CONFIG.SCREW_PITCH_Z_MM/(self.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z))))

    def set_z_limit_neg_mm(self,value_mm):
        if CONFIG.STAGE_MOVEMENT_SIGN_Z > 0:
            self.microcontroller.set_lim(LIMIT_CODE.Z_NEGATIVE,int(value_mm/(CONFIG.SCREW_PITCH_Z_MM/(self.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z))))
        else:
            self.microcontroller.set_lim(LIMIT_CODE.Z_POSITIVE,CONFIG.STAGE_MOVEMENT_SIGN_Z*int(value_mm/(CONFIG.SCREW_PITCH_Z_MM/(self.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z))))
...
class SlidePositionControlWorker(QObject):
    ...
    def wait_till_operation_is_completed(self, timestamp_start, slide_position_switching_timeout_limit_s):
    ...
    def move_to_slide_loading_position(self):
        ...
        self.wait_till_operation_is_completed(timestamp_start, CONFIG.SLIDE_POTISION_SWITCHING_TIMEOUT_LIMIT_S)
        print('z retracted')
        ...
        if self.slidePositionController.is_for_wellplate:
            ...
            # for glass slide
            if self.slidePositionController.homing_done == False or CONFIG.SLIDE_POTISION_SWITCHING_HOME_EVERYTIME:
                ...
                timestamp_start = time.time()
                self.navigationController.move_y(CONFIG.SLIDE_POSITION.LOADING_Y_MM-self.navigationController.y_pos_mm)
                self.wait_till_operation_is_completed(timestamp_start, CONFIG.SLIDE_POTISION_SWITCHING_TIMEOUT_LIMIT_S)
                self.navigationController.move_x(CONFIG.SLIDE_POSITION.LOADING_X_MM-self.navigationController.x_pos_mm)
                self.wait_till_operation_is_completed(timestamp_start, CONFIG.SLIDE_POTISION_SWITCHING_TIMEOUT_LIMIT_S)
    ...
    def move_to_slide_scanning_position(self):
        ...
        if self.slidePositionController.is_for_wellplate:
            ...
            if self.slidePositionController.homing_done == False or CONFIG.SLIDE_POTISION_SWITCHING_HOME_EVERYTIME:
                ...
                timestamp_start = time.time()
                self.navigationController.move_y(CONFIG.SLIDE_POSITION.SCANNING_Y_MM-self.navigationController.y_pos_mm)
                self.wait_till_operation_is_completed(timestamp_start, CONFIG.SLIDE_POTISION_SWITCHING_TIMEOUT_LIMIT_S)
                self.navigationController.move_x(CONFIG.SLIDE_POSITION.SCANNING_X_MM-self.navigationController.x_pos_mm)
                self.wait_till_operation_is_completed(timestamp_start, CONFIG.SLIDE_POTISION_SWITCHING_TIMEOUT_LIMIT_S)
        ...
        if self.slidePositionController.objective_retracted:
            if self.navigationController.get_pid_control_flag(2) is False:
                ...
                self.navigationController.microcontroller.move_z_to_usteps(self.slidePositionController.z_pos - CONFIG.STAGE_MOVEMENT_SIGN_Z*_usteps_to_clear_backlash)
                self.wait_till_operation_is_completed(timestamp_start, CONFIG.SLIDE_POTISION_SWITCHING_TIMEOUT_LIMIT_S)
...
class SlidePositionController(QObject):
    ...
    def __init__(self,navigationController,liveController,is_for_wellplate=False):
        ...
        self.retract_objective_before_moving = CONFIG.RETRACT_OBJECTIVE_BEFORE_MOVING_TO_LOADING_POSITION
...
class AutofocusWorker(QObject):
    ...
    def run_autofocus(self):
        ...
        for i in range(self.N):
            ...
            focus_measure = utils.calculate_focus_measure(image,CONFIG.FOCUS_MEASURE_OPERATOR)
...
class AutoFocusController(QObject):
    ...
    def set_deltaZ(self,deltaZ_um):
        mm_per_ustep_Z = CONFIG.SCREW_PITCH_Z_MM/(self.navigationController.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z)
        ...
        for coordinate_id in range(n_regions):
            ...
            for i in range(self.NY):
                ...
                for j in range(self.NX):
                    ...
                    if CONFIG.RUN_CUSTOM_MULTIPOINT and "multipoint_custom_script_entry" in globals():
                        ...
                        if self.do_reflection_af == False:
                            ...
                            if ( (self.NZ == 1) or CONFIG.Z_STACKING_CONFIG == 'FROM CENTER' ) and (self.do_autofocus) and (self.FOV_counter%CONFIG.Acquisition.NUMBER_OF_FOVS_PER_AF==0):
                            ...
                            # if (self.NZ == 1) and (self.do_autofocus):
                                ...
                                if (self.FOV_counter%CONFIG.Acquisition.NUMBER_OF_FOVS_PER_AF==0) or self.autofocusController.use_focus_map:
                            ...
                            if self.microscope.laserAutofocusController.is_initialized==False:
                                ...
                                if self.do_autofocus and ( (self.NZ == 1) or CONFIG.Z_STACKING_CONFIG == 'FROM CENTER' ) :
                                    configuration_name_AF = CONFIG.MULTIPOINT_AUTOFOCUS_CHANNEL
                                    config_AF = next((config for config in self.configurationManager.configurations if config.name == configuration_name_AF))
                                    self.signal_current_configuration.emit(config_AF)
                                    self.autofocusController.autofocus()
                                    ...
                                    if image.dtype == np.uint16:
                                        ...
                                        if self.camera.is_color:
                                            if 'BF LED matrix' in config.name:
                                                ...
                                                elif CONFIG.MULTIPOINT_BF_SAVING_OPTION == 'Green Channel Only':
                                                    image = image[:,:,1]
                                        iio.imwrite(saving_path,image)
                                    else:
                                        saving_path = os.path.join(current_path, file_ID + '_' + str(config.name).replace(' ','_') + '.' + CONFIG.Acquisition.IMAGE_FORMAT)
                                        if self.camera.is_color:
                                            if 'BF LED matrix' in config.name:
            ...
            if n_regions == 1:
                ...
                if self.navigationController.get_pid_control_flag(2) is False:
                    _usteps_to_clear_backlash = max(160,20*self.navigationController.z_microstepping)
                    self.navigationController.microcontroller.move_z_to_usteps(z_pos - CONFIG.STAGE_MOVEMENT_SIGN_Z*_usteps_to_clear_backlash)
                    self.wait_till_operation_is_completed()
                    self.navigationController.move_z_usteps(_usteps_to_clear_backlash)
                    self.wait_till_operation_is_completed()
                else:
...
class MultiPointController(QObject):
    ...
    def __init__(self,camera,navigationController,liveController,autofocusController,configurationManager,usb_spectrometer=None,scanCoordinates=None,parent=None):
        ...
        self.display_resolution_scaling = CONFIG.Acquisition.IMAGE_DISPLAY_SCALING_FACTOR
        self.counter = 0
    ...
    def start_new_experiment(self,experiment_ID): # @@@ to do: change name to prepare_folder_for_new_experiment
        ...
        acquisition_parameters = {'dx(mm)':self.deltaX, 'Nx':self.NX, 'dy(mm)':self.deltaY, 'Ny':self.NY, 'dz(um)':self.deltaZ*1000,'Nz':self.NZ,'dt(s)':self.deltat,'Nt':self.Nt,'with CONFIG.AF':self.do_autofocus,'with reflection CONFIG.AF':self.do_reflection_af}
        try: # write objective data if it is available
            ...
            try:
                ...
                pass
        acquisition_parameters['sensor_pixel_size_um'] = CONFIG.CAMERA_PIXEL_SIZE_UM[CONFIG.CAMERA_SENSOR]
...
class TrackingWorker(QObject):
    ...
    def run(self):
        ...
        while self.trackingController.flag_stop_tracking_requested == False:
            ...
            image = utils.rotate_and_flip_image(image,rotate_image_angle=CONFIG.ROTATE_IMAGE_ANGLE,flip_image=CONFIG.FLIP_IMAGE)
            # get image size
            ...
            for config_ in self.selected_configurations[1:]:
                ...
                image_ = utils.rotate_and_flip_image(image_,rotate_image_angle=CONFIG.ROTATE_IMAGE_ANGLE,flip_image=CONFIG.FLIP_IMAGE)
                # display image
                # self.image_to_display.emit(cv2.resize(image,(round(self.crop_width*self.display_resolution_scaling), round(self.crop_height*self.display_resolution_scaling)),cv2.INTER_LINEAR))
            ...
            if self.trackingController.flag_stage_tracking_enabled:
                x_correction_usteps = int(x_error_mm/(CONFIG.SCREW_PITCH_X_MM/CONFIG.FULLSTEPS_PER_REV_X/self.navigationController.x_microstepping))
                y_correction_usteps = int(y_error_mm/(CONFIG.SCREW_PITCH_Y_MM/CONFIG.FULLSTEPS_PER_REV_Y/self.navigationController.y_microstepping))
                self.microcontroller.move_x_usteps(CONFIG.TRACKING_MOVEMENT_SIGN_X*x_correction_usteps)
                self.microcontroller.move_y_usteps(CONFIG.TRACKING_MOVEMENT_SIGN_Y*y_correction_usteps) 
...
class NavigationViewer(QFrame):
    ...
    def __init__(self, sample = 'glass slide', invertX = False, *args, **kwargs):
        ...
        if sample == 'glass slide':
            self.background_image = cv2.imread(self.current_dir + '/images/slide carrier_828x662.png')
        elif sample == '384 well plate':
            self.background_image = cv2.imread(self.current_dir + '/images/384 well plate_1509x1010.png')
        elif sample == '96 well plate':
            self.background_image = cv2.imread(self.current_dir + '/images/96 well plate_1509x1010.png')
        elif sample == '24 well plate':
            self.background_image = cv2.imread(self.current_dir + '/images/24 well plate_1509x1010.png')
        elif sample == '12 well plate':
            self.background_image = cv2.imread(self.current_dir + '/images/12 well plate_1509x1010.png')
        elif sample == '6 well plate':
            self.background_image = cv2.imread(self.current_dir + '/images/6 well plate_1509x1010.png')
        ...
        if sample == 'glass slide':
            ...
            self.origin_bottom_left_x = CONFIG.X_ORIGIN_384_WELLPLATE_PIXEL - (CONFIG.X_MM_384_WELLPLATE_UPPERLEFT)/self.mm_per_pixel
            self.origin_bottom_left_y = CONFIG.Y_ORIGIN_384_WELLPLATE_PIXEL - (CONFIG.Y_MM_384_WELLPLATE_UPPERLEFT)/self.mm_per_pixel         
...
class PlateReaderNavigationController(QObject):
    ...
    def moveto(self,column,row):
        if column != '':
            mm_per_ustep_X = CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X)
            x_mm = CONFIG.PLATE_READER.OFFSET_COLUMN_1_MM + (int(column)-1)*CONFIG.PLATE_READER.COLUMN_SPACING_MM
            x_usteps = CONFIG.STAGE_MOVEMENT_SIGN_X*round(x_mm/mm_per_ustep_X)
            self.move_x_to_usteps(x_usteps)
        if row != '':
            mm_per_ustep_Y = CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y)
            y_mm = CONFIG.PLATE_READER.OFFSET_ROW_A_MM + (ord(row) - ord('A'))*CONFIG.PLATE_READER.ROW_SPACING_MM
    ...
    def moveto_row(self,row):
        ...
        mm_per_ustep_Y = CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y)
        y_mm = CONFIG.PLATE_READER.OFFSET_ROW_A_MM + row*CONFIG.PLATE_READER.ROW_SPACING_MM
    ...
    def moveto_column(self,column):
        ...
        mm_per_ustep_X = CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X)
        x_mm = CONFIG.PLATE_READER.OFFSET_COLUMN_1_MM + column*CONFIG.PLATE_READER.COLUMN_SPACING_MM
    ...
    def update_pos(self,microcontroller):
        ...
        # calculate position in mm or rad
        if CONFIG.USE_ENCODER_X:
            self.x_pos_mm = x_pos*CONFIG.STAGE_POS_SIGN_X*CONFIG.ENCODER_STEP_SIZE_X_MM
        else:
            self.x_pos_mm = x_pos*CONFIG.STAGE_POS_SIGN_X*(CONFIG.SCREW_PITCH_X_MM/(self.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X))
        if CONFIG.USE_ENCODER_Y:
            self.y_pos_mm = y_pos*CONFIG.STAGE_POS_SIGN_Y*CONFIG.ENCODER_STEP_SIZE_Y_MM
        else:
            self.y_pos_mm = y_pos*CONFIG.STAGE_POS_SIGN_Y*(CONFIG.SCREW_PITCH_Y_MM/(self.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y))
        if CONFIG.USE_ENCODER_Z:
            self.z_pos_mm = z_pos*CONFIG.STAGE_POS_SIGN_Z*CONFIG.ENCODER_STEP_SIZE_Z_MM
        else:
            self.z_pos_mm = z_pos*CONFIG.STAGE_POS_SIGN_Z*(CONFIG.SCREW_PITCH_Z_MM/(self.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z))
        # check homing status
        ...
        column = round((self.x_pos_mm - CONFIG.PLATE_READER.OFFSET_COLUMN_1_MM)/CONFIG.PLATE_READER.COLUMN_SPACING_MM)
        if column >= 0 and column <= CONFIG.PLATE_READER.NUMBER_OF_COLUMNS:
            column = str(column+1)
        else:
            column = ' '
        row = round((self.y_pos_mm - CONFIG.PLATE_READER.OFFSET_ROW_A_MM)/CONFIG.PLATE_READER.ROW_SPACING_MM)
    ...
    def get_selected_wells(self):
        ...
        for row in rows:
            ...
            for column in columns:
                x_mm = CONFIG.X_MM_384_WELLPLATE_UPPERLEFT + CONFIG.WELL_SIZE_MM_384_WELLPLATE/2 - (CONFIG.A1_X_MM_384_WELLPLATE+CONFIG.WELL_SPACING_MM_384_WELLPLATE*CONFIG.NUMBER_OF_SKIP_384) + column*CONFIG.WELL_SPACING_MM + CONFIG.A1_X_MM + CONFIG.WELLPLATE_OFFSET_X_mm
                y_mm = CONFIG.Y_MM_384_WELLPLATE_UPPERLEFT + CONFIG.WELL_SIZE_MM_384_WELLPLATE/2 - (CONFIG.A1_Y_MM_384_WELLPLATE+CONFIG.WELL_SPACING_MM_384_WELLPLATE*CONFIG.NUMBER_OF_SKIP_384) + row*CONFIG.WELL_SPACING_MM + CONFIG.A1_Y_MM + CONFIG.WELLPLATE_OFFSET_Y_mm
...
class LaserAutofocusController(QObject):
    ...
    def initialize_auto(self):
        ...
        # set camera crop
        self.initialize_manual(x_offset, y_offset, CONFIG.LASER_AF_CROP_WIDTH, CONFIG.LASER_AF_CROP_HEIGHT, 1, x)
