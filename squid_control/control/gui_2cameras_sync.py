...
class OctopiGUI(QMainWindow):
	...
	def __init__(self, is_simulation = False, *args, **kwargs):
		...
		if is_simulation:
			for i in range(len(channels)):
				self.camera[channels[i]] = camera.Camera_Simulation(sn=CAMERA_SN[channels[i]],is_global_shutter=True,rotate_image_angle=CONFIG.ROTATE_IMAGE_ANGLE,flip_image=CONFIG.FLIP_IMAGE)
			self.microcontroller = microcontroller.Microcontroller_Simulation()
			self.microcontroller2 = microcontroller2.Microcontroller2_Simulation()
		else:
			for i in range(len(channels)):
				self.camera[channels[i]] = camera.Camera(sn=CAMERA_SN[channels[i]],is_global_shutter=True,rotate_image_angle=CONFIG.ROTATE_IMAGE_ANGLE,flip_image=CONFIG.FLIP_IMAGE)
			self.microcontroller = microcontroller.Microcontroller_Simulation()
		...
		for i in range(len(channels)): 
			...
			self.streamHandler[channels[i]] = core.StreamHandler(display_resolution_scaling=CONFIG.DEFAULT_DISPLAY_CROP/100)
			...
			self.imageSaver[channels[i]] = core.ImageSaver(image_format=CONFIG.Acquisition.IMAGE_FORMAT)
			# widgets
