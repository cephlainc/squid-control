...
class Microcontroller():    
    ...
    def home_x(self):
        ...
        cmd[2] = AXIS.X
        cmd[3] = int((CONFIG.STAGE_MOVEMENT_SIGN_X+1)/2) # "move backward" if SIGN is 1, "move forward" if SIGN is -1
        self.send_command(cmd)
        # while self.mcu_cmd_execution_in_progress == True:
        #     time.sleep(self._motion_status_checking_interval)
        #     # to do: add timeout
    ...
    def home_y(self):
        ...
        cmd[1] = CMD_SET.HOME_OR_ZERO
        cmd[2] = AXIS.Y
        cmd[3] = int((CONFIG.STAGE_MOVEMENT_SIGN_Y+1)/2) # "move backward" if SIGN is 1, "move forward" if SIGN is -1
        self.send_command(cmd)
        # while self.mcu_cmd_execution_in_progress == True:
    ...
    def home_z(self):
        ...
        cmd[1] = CMD_SET.HOME_OR_ZERO
        cmd[2] = AXIS.Z
        cmd[3] = int((CONFIG.STAGE_MOVEMENT_SIGN_Z+1)/2) # "move backward" if SIGN is 1, "move forward" if SIGN is -1
        self.send_command(cmd)
        # while self.mcu_cmd_execution_in_progress == True:
        #     time.sleep(self._motion_status_checking_interval)
        #     # to do: add timeout
    ...
    def home_theta(self):
        ...
        cmd[1] = CMD_SET.HOME_OR_ZERO
        cmd[2] = 3
        cmd[3] = int((CONFIG.STAGE_MOVEMENT_SIGN_THETA+1)/2) # "move backward" if SIGN is 1, "move forward" if SIGN is -1
        self.send_command(cmd)
        # while self.mcu_cmd_execution_in_progress == True:
        #     time.sleep(self._motion_status_checking_interval)
        #     # to do: add timeout
    ...
    def home_xy(self):
        ...
        cmd[2] = AXIS.XY
        cmd[3] = int((CONFIG.STAGE_MOVEMENT_SIGN_X+1)/2) # "move backward" if SIGN is 1, "move forward" if SIGN is -1
        cmd[4] = int((CONFIG.STAGE_MOVEMENT_SIGN_Y+1)/2) # "move backward" if SIGN is 1, "move forward" if SIGN is -1
        self.send_command(cmd)
    ...
    def configure_actuators(self):
        ...
        self.configure_motor_driver(AXIS.X,CONFIG.MICROSTEPPING_DEFAULT_X,CONFIG.X_MOTOR_RMS_CURRENT_mA,CONFIG.X_MOTOR_I_HOLD)
        self.wait_till_operation_is_completed()
        self.configure_motor_driver(AXIS.Y,CONFIG.MICROSTEPPING_DEFAULT_Y,CONFIG.Y_MOTOR_RMS_CURRENT_mA,CONFIG.Y_MOTOR_I_HOLD)
        self.wait_till_operation_is_completed()
        self.configure_motor_driver(AXIS.Z,CONFIG.MICROSTEPPING_DEFAULT_Z,CONFIG.Z_MOTOR_RMS_CURRENT_mA,CONFIG.Z_MOTOR_I_HOLD)
        self.wait_till_operation_is_completed()
        # max velocity and acceleration
        self.set_max_velocity_acceleration(AXIS.X,CONFIG.MAX_VELOCITY_X_mm,CONFIG.MAX_ACCELERATION_X_mm)
        self.wait_till_operation_is_completed()
        self.set_max_velocity_acceleration(AXIS.Y,CONFIG.MAX_VELOCITY_Y_mm,CONFIG.MAX_ACCELERATION_Y_mm)
        self.wait_till_operation_is_completed()
        self.set_max_velocity_acceleration(AXIS.Z,CONFIG.MAX_VELOCITY_Z_mm,CONFIG.MAX_ACCELERATION_Z_mm)
        self.wait_till_operation_is_completed()
        # home switch
    ...
    def read_received_packet(self):
        while self.terminate_reading_received_packet_thread == False:
            ...
            if (self._cmd_id_mcu == self._cmd_id) and (self._cmd_execution_status == CONFIG.CMD_EXECUTION_STATUS.COMPLETED_WITHOUT_ERRORS):
            ...
            elif self._cmd_execution_status == CONFIG.CMD_EXECUTION_STATUS.CMD_CHECKSUM_ERROR:
            ...
            self.x_pos = self._payload_to_int(msg[2:6],CONFIG.MicrocontrollerDef.N_BYTES_POS) # unit: microstep or encoder resolution
            self.y_pos = self._payload_to_int(msg[6:10],CONFIG.MicrocontrollerDef.N_BYTES_POS) # unit: microstep or encoder resolution
            self.z_pos = self._payload_to_int(msg[10:14],CONFIG.MicrocontrollerDef.N_BYTES_POS) # unit: microstep or encoder resolution
            self.theta_pos = self._payload_to_int(msg[14:18],CONFIG.MicrocontrollerDef.N_BYTES_POS) # unit: microstep or encoder resolution
    ...
    def read_received_packet(self):
        while self.terminate_reading_received_packet_thread == False:
            ...
            if time.time() - self.timestamp_last_command > 0.05: # in the simulation, assume all the operation takes 0.05s to complete
                if self._mcu_cmd_execution_status !=  CONFIG.CMD_EXECUTION_STATUS.COMPLETED_WITHOUT_ERRORS:
