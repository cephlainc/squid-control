...
class LiveControlWidget(QFrame):
    ...
    def add_components(self,show_trigger_options,show_display_options,show_autolevel,autolevel):
        ...
        self.dropdown_triggerManu.addItems([TriggerModeSetting.SOFTWARE.value,TriggerModeSetting.HARDWARE.value,TriggerModeSetting.CONTINUOUS.value])
...
class NavigationWidget(QFrame):
    ...
    def set_deltaX(self,value):
        mm_per_ustep = CONFIG.SCREW_PITCH_X_MM/(self.navigationController.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X) # to implement a get_x_microstepping() in multipointController
        deltaX = round(value/mm_per_ustep)*mm_per_ustep
        self.entry_dX.setValue(deltaX)
    def set_deltaY(self,value):
        mm_per_ustep = CONFIG.SCREW_PITCH_Y_MM/(self.navigationController.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y)
        deltaY = round(value/mm_per_ustep)*mm_per_ustep
        self.entry_dY.setValue(deltaY)
    def set_deltaZ(self,value):
        mm_per_ustep = CONFIG.SCREW_PITCH_Z_MM/(self.navigationController.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z)
...
class AutoFocusWidget(QFrame):
    ...
    def set_deltaZ(self,value):
        mm_per_ustep = CONFIG.SCREW_PITCH_Z_MM/(self.autofocusController.navigationController.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z)
        deltaZ = round(value/1000/mm_per_ustep)*mm_per_ustep*1000
...
class MultiPointWidget(QFrame):
    ...
    def add_components(self):
        ...
        self.checkbox_withAutofocus.setChecked(CONFIG.MULTIPOINT_AUTOFOCUS_ENABLE_BY_DEFAULT)
        self.multipointController.set_af_flag(CONFIG.MULTIPOINT_AUTOFOCUS_ENABLE_BY_DEFAULT)
        ...
        self.checkbox_withReflectionAutofocus.setChecked(CONFIG.MULTIPOINT_REFLECTION_AUTOFOCUS_ENABLE_BY_DEFAULT)

        self.multipointController.set_reflection_af_flag(CONFIG.MULTIPOINT_REFLECTION_AUTOFOCUS_ENABLE_BY_DEFAULT)
    ...
    def set_deltaX(self,value):
        mm_per_ustep = CONFIG.SCREW_PITCH_X_MM/(self.multipointController.navigationController.x_microstepping*CONFIG.FULLSTEPS_PER_REV_X) # to implement a get_x_microstepping() in multipointController
        deltaX = round(value/mm_per_ustep)*mm_per_ustep
        self.entry_deltaX.setValue(deltaX)
        self.multipointController.set_deltaX(deltaX)

    def set_deltaY(self,value):
        mm_per_ustep = CONFIG.SCREW_PITCH_Y_MM/(self.multipointController.navigationController.y_microstepping*CONFIG.FULLSTEPS_PER_REV_Y)
        deltaY = round(value/mm_per_ustep)*mm_per_ustep
        self.entry_deltaY.setValue(deltaY)
        self.multipointController.set_deltaY(deltaY)

    def set_deltaZ(self,value):
        mm_per_ustep = CONFIG.SCREW_PITCH_Z_MM/(self.multipointController.navigationController.z_microstepping*CONFIG.FULLSTEPS_PER_REV_Z)
        deltaZ = round(value/1000/mm_per_ustep)*mm_per_ustep*1000
...
class TrackingControllerWidget(QFrame):
    ...
    def update_pixel_size(self): 
        ...
        pixel_size_um = CONFIG.CAMERA_PIXEL_SIZE_UM[CONFIG.CAMERA_SENSOR] / ( CONFIG.TUBE_LENS_MM/ (CONFIG.OBJECTIVES[objective]['tube_lens_f_mm']/CONFIG.OBJECTIVES[objective]['magnification']) )
...
class PlateReaderNavigationWidget(QFrame):
    ...
    def add_components(self):
        ...
        self.dropdown_column.addItems([str(i+1) for i in range(CONFIG.PLATE_READER.NUMBER_OF_COLUMNS)])
        self.dropdown_row = QComboBox()
        self.dropdown_row.addItems([''])
        self.dropdown_row.addItems([chr(i) for i in range(ord('A'),ord('A')+CONFIG.PLATE_READER.NUMBER_OF_ROWS)])
...
class TriggerControlWidget(QFrame):
    ...
    def add_components(self):
        ...
        self.triggerMode = None
        self.dropdown_triggerManu = QComboBox()
        self.dropdown_triggerManu.addItems([TriggerModeSetting.SOFTWARE,TriggerModeSetting.HARDWARE])
...
class WellSelectionWidget(QTableWidget):
    ...
    def onDoubleClick(self,row,col):
        if (row >= 0 + CONFIG.NUMBER_OF_SKIP and row <= self.rows-1-CONFIG.NUMBER_OF_SKIP ) and ( col >= 0 + CONFIG.NUMBER_OF_SKIP and col <= self.columns-1-CONFIG.NUMBER_OF_SKIP ):
            x_mm = CONFIG.X_MM_384_WELLPLATE_UPPERLEFT + CONFIG.WELL_SIZE_MM_384_WELLPLATE/2 - (CONFIG.A1_X_MM_384_WELLPLATE+CONFIG.WELL_SPACING_MM_384_WELLPLATE*CONFIG.NUMBER_OF_SKIP_384) + col*CONFIG.WELL_SPACING_MM + CONFIG.A1_X_MM + CONFIG.WELLPLATE_OFFSET_X_mm
