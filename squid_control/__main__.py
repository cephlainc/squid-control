...
def main():
    ...
    os.environ["QT_QPA_PLATFORM_PLUGIN_PATH"] = os.path.join(os.path.dirname(sys.executable), "lib", "python" + sys.version[:3], "site-packages", "PyQt5", "Qt", "plugins")
    app = QApplication([])
    app.setStyle('Fusion')
    if(args.simulation):
    ...
    if not legacy_config:
        ...
        config_action.triggered.connect(lambda : show_config(cf_editor_parser, args.config, win))
